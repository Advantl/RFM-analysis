...
## Источник даннных
Набор данных предоставлен образовательной платформой Simulative в рамках Курса-симулятора «Аналитик данных» Simulative (ООО «АЙТИ РЕЗЮМЕ») https://simulative.ru/data-analyst. Датасет представляет собой реальные исторические данные о продажах 8 аптек, расположенных в различных районах одного и того же города. 

## Цель проекта
Главной целью данной работы является проведение сегментации клиентской базы на основе анализа паттернов их покупательского поведения. А также разработка конкретных рекомендации и персонализированных предложений для каждой из выявленных групп потребителей.
